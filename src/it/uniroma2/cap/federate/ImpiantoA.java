		...
		try {
			while(true) {
				...
				if (!eventsList.isEmpty()) {				
					...
					//processa l'evento
					event = getNextEvent();	
					//invia l'evento remoto che comunica ad M che il report è stato ricevuto
					if (event.getEventType().toString().equals("RECEIVE_REPORT_M")) {
						EventoRemotoA fine = new EventoRemotoA(EventType.REPORT_RECEIVED,fedAmbassador.getFederateTime()+1);
						sendRemoteEvent(fine);
					}
					process(event);
	...
	private void sendRemoteEvent(EventoRemotoA e) {
		...
		case REPORT_RECEIVED:
			try {
			    ...
			    HLAunicodeString typeEncoder = encoderFactory.createHLAunicodeString();
			    typeEncoder.setValue(e.getEventType().name());
			    //il tipo dell'evento viene aggiunto ai parametri dell'interazione
			    parameters.put(this.typeHandleA, typeEncoder.toByteArray());
			    HLAinteger64Time time = timeFactory.makeTime(e.getTime());
				//viene inviata l'interazione
				rtiAmb.sendInteraction(icRemoteEventAHandle, parameters, null, time);
				System.out.println("[" + fedAmbassador.federateTime + "] " + federateName + ": Evento Remoto che comunica a M la ricezione del report" );
				break;
			} catch(Exception ex) {ex.printStackTrace();}
		}
