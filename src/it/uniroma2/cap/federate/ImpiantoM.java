...
public class ImpiantoM {
	...
	private boolean reportInviato; //true se il report è stavo inviato ad A

	//simulation properties
	private long mediaM1;
	private long varianzaM1;
	private long costoM1;
	private long mediaM2;
	private long varianzaM2;
	private long costoM2;
	...
	public ImpiantoM(String nome, long mediaM1, long varianzaM1, long costoM1, long mediaM2, long varianzaM2, long costoM2) {
		this.nome = nome;
		eventsList = new TreeSet<EventoLocaleM>(); 
		federateName = "ImpiantoM";
		this.mediaM1 = mediaM1;
		this.varianzaM1 = varianzaM1;
		this.costoM1 = costoM1;
		this.mediaM2 = mediaM2;
		this.varianzaM2 = varianzaM2;
			...
			while(true) {
				...
				if (!eventsList.isEmpty()) {
					...
					while(fedAmbassador.isAdvancing)
						Thread.sleep(10);
					System.out.println("[" + fedAmbassador.getFederateTime() + "] " + federateName +"_FEDAMB TAG to " + fedAmbassador.getFederateTime());
					System.out.println("[" + fedAmbassador.federateTime + "] " + federateName + ": Time Advance Grant");
					//processa l'evento
					event = getNextEvent();	
					//se ImpiantoA ha ricevuto il report M termina la simulazione
					if (event.getEventType().toString().equals("REPORT_RECEIVED")) break;
					//process event
					process(event);
					//rimuove l'evento dalla lista degli eventi da processare
					eventsList.remove(event);
				}
				else if (numeroPezziCostruiti == listaPezzi.size() && listaPezzi.size() > 0 && !reportInviato) {	
					//creazione del report
					createReport();
					//invia report a ImpiantoA
					nextTime = currentTime + (long)pezziCostruiti.get(listaPezzi.size()-1).getDurata()+1;
					EventoRemotoM e= new EventoRemotoM(EventType.SEND_REPORT, nextTime, reportM);
					sendRemoteEvent(e);
					reportInviato=true;
				}
				else{					
		...
		case STARTING_REQUEST:
			...
			*/
			for(String p : listaPezzi) {
				...
				if( p.equals("M1")){
					Pezzo pezzoM1 = new Pezzo("M1", ExponentialDistribution(mediaM1,varianzaM1)+1, (int) costoM1);
					...
					nextEventTime = nextEventTime + (long)pezzoM1.getDurata();
					
					
				//se il pezzo è di tipo M2 avrà un durata aleatoria esponenziale di media 40 e varianza 2 e un costo fisso di 4000		
				}else{
					Pezzo pezzoM2 = new Pezzo("M2", ExponentialDistribution(mediaM2,varianzaM2)+1, (int) costoM2);
					...
					nextEventTime = nextEventTime + (long)pezzoM2.getDurata();						
				}					
			}
			break;
		}		
